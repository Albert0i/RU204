...
def dump(redis, filename="/data/ru101.json", compress=False, match="*"):
  ...
  try:
    ...
    for k in redis.scan_iter(match=match, count=1000):
      obj = {}
      t = redis.type(k).decode("utf-8")
      obj['t'] = t
      obj['k'] = k.decode("utf-8")
      obj['ttl'] = redis.ttl(k)
      if t == "hash":
        ...
        obj['e'] = encoding.decode("utf-8")
        if encoding == b"embstr":
          obj['v'] = redis.get(k).decode("utf-8")
        elif encoding == b"raw":
      ...
      # All regular expressions are generated by ChatGPT
